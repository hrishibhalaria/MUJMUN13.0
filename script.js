...
(function () {
    ...
    /* ══════════════════════════════════════════════════════════════
       SOUNDTRACK + ANALYSER
       (Audio init is now inlined in startExperience for safety)
       ══════════════════════════════════════════════════════════════ */
    ...
    class MissileSystem {
        ...
        spawn(fromLeft) {
            ...
            if (this.missiles.filter(m => m.active).length >= maxActive) return;
    ...
    /* ══════════════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════════════ */
    function setSmoke(op) {
        smokeCanvas.style.opacity = (isMobile ? Math.min(op * 2, 1.0) : op).toFixed(2);
    }
    ...
    function renderLoop(now) {
        ...
        const maxMap = isMobile ? 0.25 : 0.18;
        if (!mapReady) {
            // Slow initial fade-in
            mapOpacity += 0.00025;
            if (mapOpacity >= maxMap) {
                mapOpacity = maxMap;
                mapReady = true;
            }
            ...
            const dynamicAlpha = maxMap + energy.overall * 0.10;
            wmCtx.clearRect(0, 0, W, H);
            wmCtx.globalAlpha = Math.min(dynamicAlpha, 1);
            wmCtx.drawImage(worldMapImg, 0, 0);
            wmCtx.globalAlpha = 1;
        }
    ...
    function init() {
        ...
        // Pre-create the audio element, but don't play yet
        ...
        const begin = (e) => {
            ...
            try {
                ...
                // If startExperience itself throws, still start visuals
                console.error('startExperience error:', err);
                startVisualsOnly();
            }
        };
    ...
    // Fallback: start visuals without audio if everything else fails
    function startVisualsOnly() {
        if (!smoke) smoke = new SmokeSystem(smCtx, W, H);
        if (!missiles) missiles = new MissileSystem(msCtx, W, H);
        if (!animFrameId) animFrameId = requestAnimationFrame(renderLoop);
        scheduleTimeline();
    }

    function startExperience() {
        // ── 1. Set up Web Audio analyser — DESKTOP ONLY ──
        // On mobile, createMediaElementSource() hijacks the audio output
        // and routes it through AudioContext.destination, which often
        // doesn't connect to device speakers — causing "playing but silent."
        // So on mobile we skip Web Audio entirely and let the Audio element
        // play directly through the phone's speakers.
        ...
        if (!isTouchDevice) {
            ...
            // Resume AudioContext if suspended (some desktop browsers)
            if (audioCtx && audioCtx.state === 'suspended') {
                audioCtx.resume().catch(e => console.warn('AudioContext resume failed:', e));
            }
        }

        // ── 2. Play soundtrack (fire-and-forget, never blocks visuals) ──
        // On mobile this plays directly through the phone speakers (no Web Audio hijack).
        soundtrack.play().catch(err => {
            console.warn('Initial play blocked, will retry on next tap:', err);
            const retryPlay = () => {
        ...
        // ── 3. ALWAYS start visuals — this must never be blocked by audio ──
